...
### Documentation
- Standardized README examples on the `telegram-personal` channel and `urgent` profile names emitted by `agent-notify init`, and added a test that fails when the generated sample config defines a channel or profile the README does not document.
